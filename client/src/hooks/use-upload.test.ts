...
describe('useUpload', () => {
  ...
  describe('uploadFile', () => {
    ...
    it('should upload file to presigned URL with correct headers', async () => {
      ...
      expect(global.fetch).toHaveBeenNthCalledWith(2, 'https://storage.example.com/presigned-url', {
        method: 'PUT',
        body: mockFile,
        headers: {
  ...
  describe('getUploadParameters', () => {
    ...
    it('should throw error when presigned URL request fails', async () => {
      ...
      await expect(result.current.getUploadParameters(mockUppyFile)).rejects.toThrow(
