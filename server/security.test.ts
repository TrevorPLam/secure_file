...
import { securityHeaders, corsPolicy, createRateLimiter, applySecurityMiddleware } from './security'
...
describe('Security Middleware', () => {
  ...
  describe('corsPolicy', () => {
    ...
    it('should handle OPTIONS preflight requests', async () => {
      const response = await request(app).options('/test').set('Origin', 'http://localhost:5000')
    ...
    it('should set CORS headers for allowed origins', async () => {
      ...
      const response = await request(app).get('/test').set('Origin', 'http://localhost:5000')
