...
export class DatabaseStorage implements IStorage {
  ...
  async getShareLinksByFile(fileId: string): Promise<ShareLink[]> {
    return db.select().from(shareLinks).where(
      and(eq(shareLinks.fileId, fileId), eq(shareLinks.isActive, true))
    );
  }
  ...
  async getStats(userId: string): Promise<{ totalFiles: number; totalFolders: number; totalSize: number }> {
